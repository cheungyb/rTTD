...
*** and has linear effect on ln(HR)

cd "C:\working"
  
capture log close
log using rttd_linear,text replace
...
capture program drop rttd_l
program define rttd_l,rclass
...
gen eventLogOdds=-8+ln(`hr')*trt-2*zt+x3
...
simulate, reps(1000) seed(123) dots(10): rttd_l, hr($hr) r(0.9)
...
simulate, reps(1000) seed(123) dots(10): rttd_l, hr($hr) r(0.5)
qui foreach m in crude x zt {
gen rttd_`m'_rmse=(exp(rttd_`m'_est)-$hr)^2
sum rttd_`m'_rmse
replace rttd_`m'_rmse=sqrt(r(mean))
...
simulate, reps(1000) seed(123) dots(10): rttd_l, hr($hr) r(0.1)
qui foreach m in crude x zt {
gen rttd_`m'_rmse=(exp(rttd_`m'_est)-$hr)^2
sum rttd_`m'_rmse
replace rttd_`m'_rmse=sqrt(r(mean))
