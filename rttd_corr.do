...
*** and different levels of correlation with mortality 

cd "C:\working"
 
capture log close
log using rttd_corr,text replace
...
simulate, reps(1000) seed(123) dots(10): rttd_pwl, hr($hr) corr(0.5)
...
simulate, reps(1000) seed(123) dots(10): rttd_pwl, hr($hr) corr(0.25)
qui foreach m in crude x zt {
gen rttd_`m'_rmse=(exp(rttd_`m'_est)-$hr)^2
sum rttd_`m'_rmse
replace rttd_`m'_rmse=sqrt(r(mean))
...
simulate, reps(1000) seed(123) dots(10): rttd_pwl, hr($hr) corr(0)
qui foreach m in crude x zt {
gen rttd_`m'_rmse=(exp(rttd_`m'_est)-$hr)^2
sum rttd_`m'_rmse
replace rttd_`m'_rmse=sqrt(r(mean))
